...
import { Bookmark, ChevronDown, Copy, Download, Languages, Mail, MessageCircle, Minus, Minimize2, Maximize2, Pause, Play, Plus, Share2, SkipBack, SkipForward, Sparkles, Square, Volume2 } from "lucide-react";
import { toast } from "sonner";
import { verses, verseCount } from "@/lib/hanumanChalisa";
import { marathiMeanings } from "@/lib/marathiChalisa";
import { generatedMeaningReviewNote, generatedMeanings } from "@/lib/generatedMeanings";
...
export default function Home() {
  const [language, setLanguage] = useState(() => {
    ...
    const stored = window.localStorage.getItem("chalisa-language");
    return stored && languages.some((item) => item.id === stored) ? stored : "hi";
  });
  ...
  const getMeaning = (verse: typeof verses[number], index: number) => {
    if (language === "en") return verse.en;
    if (language === "mr") return marathiMeanings[index] ?? verse.en;
    const generated = generatedMeanings[language];
    if (language === "roman") {
      const hindiMeaning = generatedMeanings.hi?.[index];
      return hindiMeaning ? Sanscript.t(hindiMeaning, "devanagari", "itrans") : verse.en;
    }
    return generated?.[index] ?? verse.en;
  };

  const meaningIsMachineGenerated = !["en", "mr"].includes(language);
  ...
  useEffect(() => {
    window.localStorage.setItem("chalisa-language", language);
  }, [language]);
